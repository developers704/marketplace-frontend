...
import ResetPasswordForm from '@components/auth/reset-pasword-form';
const LoginForm = dynamic(() => import('@components/auth/login-form'));
const SignUpForm = dynamic(() => import('@components/auth/sign-up-form'));
const ForgetPasswordForm = dynamic(
  () => import('@components/auth/forget-password-form'),
);
const OTPVerifications = dynamic(
  () => import('@components/auth/otp-verifications'),
);
...
export default function ManagedModal({ lang }: { lang: string }) {
  ...
  return (
    <Modal open={isOpen} onClose={closeModal}>
      ...
      {view === 'RESET_PASSWORD' && <ResetPasswordForm lang={lang} />}
      ...
      {view === 'OTP_VERIFICATION' && <OTPVerifications lang={lang} />}
    </Modal>
  );
}
