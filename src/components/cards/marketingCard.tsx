...
//   console.log(data, 'marketing card');
  ...
  return (
    <div className="cursor-pointer group relative w-full flex flex-col py-4 transition ease-in-out duration-150 hover:shadow-md hover:shadow-neutral-300 rounded-lg border-[0.5px] border-gray-300">
      ...
      <Link
        ...
        href={`/${lang}/marketing/${data?._id}`}
        id="top"
        className="w-full h-[250px] object-contain relative"
      >
        {/* `${BASE_API}/${data?.image}` ||  */}
        <Image
          src={getImageUrl(BASE_API, data?.image, `/assets/images/inventory/item2.png`)}
          alt="item1"
          fill
          objectFit="contain"
        />
      ...
      >
        ...
        <div className="flex flex-col gap-2">
          <div className="text-2xl font-bold text-brand-blue">
            <span className="">$ {data?.prices[0]?.amount}</span>
          </div>
        </div>
      </Link>
    </div>
