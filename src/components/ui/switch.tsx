  ...
  ({ srText = 'toggle', checked, onChange }, ref) => {
    return (
      <Switch
        ...
        type="button"
        ref={ref}
        className={`${checked ? 'bg-brand-button-hover' : 'bg-fill-four'}
          relative inline-flex shrink-0 h-6 lg:h-7 w-10 lg:w-12 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus-visible:ring-2  focus-visible:ring-brand-light focus-visible:ring-opacity-75 focus:border-brand-button-hover`}
      >
