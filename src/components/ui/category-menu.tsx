    ...
    <>
      <li
        className={`flex justify-between items-center transition ${className
          ? className
          : 'text-sm hover:text-brand-button-hover px-3.5 2xl:px-4 py-2.5 border-b border-border-base last:border-b-0'
          }`}
      >
        <Link
              ...
              {items?.map((currentItem) => {
                ...
                return (
                  <SidebarMenuItem
                    ...
                    depth={childDepth}
                    className={cn(
                      'text-sm px-3 py-3 ltr:pr-3 rtl:pl-3 text-brand-muted hover:text-brand-button-hover border-b border-border-base last:border-b-0 mb-0.5',
                    )}
                  />
                );
              })}
