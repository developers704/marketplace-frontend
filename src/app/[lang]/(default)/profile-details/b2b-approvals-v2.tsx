...
import { CheckCircle, Eye, XCircle, Package, Store, User, DollarSign } from 'lucide-react';
...
const B2BApprovalsV2 = ({ lang }: { lang: string }) => {
  ...
  const [filteredRequests, setFilteredRequests] = useState<B2BPurchaseRequest[]>([]);
  ...
  const [selectedRequest, setSelectedRequest] = useState<B2BPurchaseRequest | null>(null);
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_API}/api/v2/b2b/requests`, {
        headers: {
  ...
  const calculateStats = (list: B2BPurchaseRequest[]) => {
    const pending = list.filter((r) => r.status === 'PENDING_DM' || r.status === 'PENDING_CM').length;
  ...
  const applyFilters = (list: B2BPurchaseRequest[], filterType: string, search: string) => {
    let filtered = list;

    if (filterType !== 'all') {
      if (filterType === 'pending') {
        filtered = filtered.filter((r) => r.status === 'PENDING_DM' || r.status === 'PENDING_CM');
  ...
  const handleApprovalSubmit = async () => {
    ...
    try {
      ...
      const method = 'POST';
      const body = approvalAction === 'REJECT' ? JSON.stringify({ reason: remarks || 'No reason provided' }) : undefined;

      const response = await fetch(`${BASE_API}/api/v2/b2b/${endpoint}/${selectedRequest._id}`, {
        method,
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, { bg: string; text: string; label: string }> = {
      PENDING_DM: { bg: 'bg-yellow-100', text: 'text-yellow-800', label: 'Pending DM' },
      PENDING_CM: { bg: 'bg-blue-100', text: 'text-blue-800', label: 'Pending CM' },
      PENDING_ADMIN: { bg: 'bg-purple-100', text: 'text-purple-800', label: 'Pending Admin' },
    ...
    return (
      <span className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full ${style.bg} ${style.text}`}>
        {style.label}
      </span>
    );
  };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto bg-gray-50 min-h-screen">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">B2B Purchase Approvals (v2)</h1>
        <p className="text-sm text-gray-600">Review and approve/reject B2B purchase requests from store managers</p>
      </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">SKU Details</th>
                ...
                filteredRequests?.map((request) => (
                  <tr key={request._id} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      <div>
                        <p className="font-medium text-gray-900">{request.vendorProductId?.title || 'N/A'}</p>
                        <p className="text-sm text-gray-500">{request.vendorProductId?.vendorModel || 'N/A'}</p>
                        {request.vendorProductId?.brand && (
                          <p className="text-xs text-gray-400">Brand: {request.vendorProductId.brand}</p>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm">
                      <div>
                        <p className="font-medium">{request.skuId?.sku || 'N/A'}</p>
                        <p className="text-gray-600">
                          {[request.skuId?.metalColor, request.skuId?.metalType, request.skuId?.size]
                            .filter(Boolean)
                            .join(' • ')}
                        </p>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium">{request.storeWarehouseId?.name || 'N/A'}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 font-semibold text-gray-900">{request.quantity}</td>
                    <td className="px-6 py-4">
                      <div>
                        <p className="font-medium">{request.requestedByUser?.username || 'N/A'}</p>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        {(request.status === 'PENDING_DM' || request.status === 'PENDING_CM') && (
                          <>
                            <button
                              onClick={() => {
                                setSelectedRequest(request);
            ...
            <div className="p-6 border-b">
              <div className="flex justify-between items-center">
                <h3 className="text-xl font-bold">
                  {approvalAction ? (approvalAction === 'APPROVE' ? 'Approve' : 'Reject') : 'View'} B2B Purchase Request
                </h3>
                <button
                  onClick={() => {
                    setShowApprovalModal(false);
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="border rounded-lg p-4 space-y-2">
                  <p>
                    <strong>Title:</strong> {selectedRequest.vendorProductId?.title || 'N/A'}
                  </p>
                  <p>
                    <strong>Vendor Model:</strong> {selectedRequest.vendorProductId?.vendorModel || 'N/A'}
                  </p>
                  {selectedRequest.vendorProductId?.brand && (
                    <p>
                      <strong>Brand:</strong> {selectedRequest.vendorProductId.brand}
                    </p>
                  )}
                  {selectedRequest.vendorProductId?.category && (
                    <p>
                      <strong>Category:</strong> {selectedRequest.vendorProductId.category}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="border rounded-lg p-4 space-y-2">
                  <p>
                    <strong>SKU:</strong> {selectedRequest.skuId?.sku || 'N/A'}
                  </p>
                  <p>
                    <strong>Price:</strong> {selectedRequest.skuId?.currency || '$'}
                  ...
                  {selectedRequest.skuId?.metalType && (
                    <p>
                      <strong>Metal Type:</strong> {selectedRequest.skuId.metalType}
                    </p>
                  )}
                  {selectedRequest.skuId?.size && (
                    <p>
                      <strong>Size:</strong> {selectedRequest.skuId.size}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="border rounded-lg p-4 space-y-1 text-sm">
                    <p>
                      <strong>Store:</strong> {selectedRequest.storeWarehouseId?.name || 'N/A'}
                    </p>
                    <p>
                ...
                <div>
                  ...
                  <div className="border rounded-lg p-4 space-y-1 text-sm">
                    <p>
                      <strong>Name:</strong> {selectedRequest.requestedByUser?.username || 'N/A'}
                    </p>
                    {selectedRequest.requestedByUser?.email && (
                      <p>
                        <strong>Email:</strong> {selectedRequest.requestedByUser.email}
                      </p>
                    )}
                    {selectedRequest.requestedByUser?.phone_number && (
                      <p>
                        <strong>Phone:</strong> {selectedRequest.requestedByUser.phone_number}
                      </p>
                    )}
                    <p>
              ...
              {/* Approval History */}
              {(selectedRequest.approvals?.dm || selectedRequest.approvals?.cm || selectedRequest.approvals?.admin) && (
                <div>
                  <h4 className="font-semibold mb-3">Approval History</h4>
                  <div className="border rounded-lg p-4 space-y-2">
                    {selectedRequest.approvals?.dm && (
                      <p className="text-sm">
                        <strong>DM Approved:</strong> {new Date(selectedRequest.approvals.dm.approvedAt).toLocaleString()}
                      </p>
                    )}
                    {selectedRequest.approvals?.cm && (
                      <p className="text-sm">
                        <strong>CM Approved:</strong> {new Date(selectedRequest.approvals.cm.approvedAt).toLocaleString()}
                      </p>
                    )}
                    {selectedRequest.approvals?.admin && (
                      <p className="text-sm">
                        <strong>Admin Approved:</strong> {new Date(selectedRequest.approvals.admin.approvedAt).toLocaleString()}
                      </p>
                    )}
                  </div>
                </div>
              ...
              {/* Rejection Info */}
              {selectedRequest.rejection && (
                <div>
                  <h4 className="font-semibold mb-3 text-red-600">Rejection Details</h4>
                  <div className="border border-red-200 rounded-lg p-4 bg-red-50">
                    <p className="text-sm">
                      <strong>Rejected At:</strong> {new Date(selectedRequest.rejection.rejectedAt).toLocaleString()}
                    </p>
                    <p className="text-sm">
                      <strong>Reason:</strong> {selectedRequest.rejection.reason || 'No reason provided'}
                    </p>
                  </div>
                </div>
              )}
